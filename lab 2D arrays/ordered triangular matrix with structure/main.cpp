 ...
 * Created on March 2, 2023, 9:42 PM
...
void mrkSort(TriMatx *triMt){
    for (int i=0; i<triMt->size-1; i++){
        for (int j=i+1; j<triMt->size; j++){
            if (triMt->col[triMt->indx[i]]>triMt->col[triMt->indx[j]]){
                triMt->indx[i] = triMt->indx[i]^triMt->indx[j];
                triMt->indx[j] = triMt->indx[i]^triMt->indx[j];
                triMt->indx[i] = triMt->indx[i]^triMt->indx[j];
            }
        }
    }
